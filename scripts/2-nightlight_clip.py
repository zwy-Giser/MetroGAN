...
#读取文件
# built area
built_area=rasterio.open(r"C:\research\city_architecture\data\original data\nightlight\2000\F142000.v4b_web.stable_lights.avg_vis.tif")
year=2000
...
def clip_pic(cities,raster_data,w,out_filepath):
	...
	for index,city in cities.iterrows():
		...
		file_path=out_filepath+"Y{year}-{id}-{country}-{province}-{city}".format(year=year,id=index,country=city["country"],province=city["province"],city=city["city"])
		print(index)
...
clip_pic(cities,built_area,100,"..\\multi-year dataset\\nightlight\\")
